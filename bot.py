...
@bot.tree.command(name="deploy", description="Deploy a VPS with custom specifications.")
...
async def deploy(interaction: discord.Interaction, player: str, cpu: int, ram: int, disk: int):
    await interaction.response.defer()
    userid = str(interaction.user.id)
    ...
    try:
        container_id = subprocess.check_output([
            ...
            "--hostname", f"{player}-vps",
            "--cpus", str(cpu),
            "--memory", f"{ram}g",
            "--storage-opt", f"size={disk}G",
            "--cap-add=ALL", image
        ]).strip().decode('utf-8')
    except subprocess.CalledProcessError as e:
    ...
    try:
        ...
        if ssh_session_line:
            ...
            await interaction.user.send(embed=discord.Embed(title="VPS Created", description=vps_details, color=0x00ff00))
            add_to_database(userid, container_id, ssh_session_line)
            ...
            await interaction.followup.send(embed=discord.Embed(description="Error: Failed to retrieve SSH details.", color=0xff0000))
            subprocess.run(["docker", "kill", container_id])
            subprocess.run(["docker", "rm", container_id])
    except subprocess.CalledProcessError as e:
        await interaction.followup.send(embed=discord.Embed(description=f"Error starting VPS: {e}", color=0xff0000))
        subprocess.run(["docker", "kill", container_id])
        subprocess.run(["docker", "rm", container_id])
